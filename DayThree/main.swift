...
func parseInput(_ input: String) -> [[String]] {
    ...
    for line in lines {
        let str = String(line)
        engine.append(Array(str).map({String($0)}))
    }
    return engine
}

func main(isFirst: Bool) {
    let engine = parseInput(input)
    let engineCount = engine.count
    let lineWidth = engine.first!.count
    
    
    var blockedIndices: [IndexPair] = []
    var sum: Int = 0
    
    if isFirst {
        for lineIndex in 0..<engineCount {
            for colIndex in 0..<lineWidth {
                let val = engine[lineIndex][colIndex]
                if !"0123456789".contains(val) {
                    continue
                }
                if !blockedIndices.contains(IndexPair(x: colIndex, y: lineIndex)) && numberHasSurroundingChar(pos: IndexPair(x: colIndex, y: lineIndex), engine: engine, maxWidth: lineWidth-1, maxHeight: engineCount-1) {
                    guard let res = extractNumberFromLine(line: engine[lineIndex], currentIndex: colIndex) else {
                        continue
                    }
                    sum += res.0
                    for index in res.1.x...res.1.y {
                        blockedIndices.append(IndexPair(x: index, y: lineIndex))
                    }
                    //print(blockedIndices)
                }
            }
        }
        print(sum)
    ...
    for lineIndex in 0..<engineCount {
        for colIndex in 0..<lineWidth {
            ...
            if !blockedIndices.contains(IndexPair(x: colIndex, y: lineIndex)), let currentStars = numberHasSurroundingStar(pos: IndexPair(x: colIndex, y: lineIndex), engine: engine, maxWidth: lineWidth-1, maxHeight: engineCount-1) {
                guard let res = extractNumberFromLine(line: engine[lineIndex], currentIndex: colIndex) else {
                    continue
                }
                //sum += res.0
                for star in currentStars {
                    if !stars.contains(where: {
                        $0.pos == star
                    }) {
                        stars.append(Star(pos: star, numbers: [res.0]))
                    } else {
                        let ind = stars.firstIndex(where: {
                            $0.pos == star
                        })!
                        stars[ind].numbers.append(res.0)
                    }
                }
    ...
    let valid = stars.compactMap({
        if $0.numbers.count == 2 {
            return $0
        }
        return nil
    })
...
func keepInRange(_ value: Int, max: Int) -> Int {
    value <= 0 ? 0 : value >= max ? max: value
}

func numberHasSurroundingChar(pos: IndexPair, engine: [[String]], maxWidth: Int, maxHeight: Int) -> Bool {
    let indexSets = {
        var pairs: [IndexPair] = []
        for i in -1...1 {
            for x in -1...1 {
                pairs.append(IndexPair(x: keepInRange(pos.x + i, max: maxWidth), y: keepInRange(pos.y + x, max: maxHeight)))
            }
        }
        return Set(pairs)
    }()
    ...
    for indexSet in indexSets {
        isSymbol.append(!".0123456789".contains(engine[indexSet.y][indexSet.x]))
    }
    return isSymbol.contains(true)
}

func numberHasSurroundingStar(pos: IndexPair, engine: [[String]], maxWidth: Int, maxHeight: Int) -> [IndexPair]? {
    let indexSets = {
        var pairs: [IndexPair] = []
        for i in -1...1 {
            for x in -1...1 {
    ...
    for indexSet in indexSets {
        if engine[indexSet.y][indexSet.x] != "*" { continue }
        stars.append(indexSet)
    }
    return stars.count == 0 ? nil: stars
}

func extractNumberFromLine(line: [String], currentIndex: Int) -> (Int, IndexPair)? {
    var startIndex = 0
    var endIndex = 0
    for offset in 0...currentIndex {
        if !"0123456789".contains(line[currentIndex-offset]) {
            startIndex = currentIndex - offset + 1
            break
        } else if offset == currentIndex {
            startIndex = 0
        }
    }
    for offset in 0..<line.count-currentIndex {
        if !"0123456789".contains(line[currentIndex+offset]) {
            endIndex = currentIndex + offset - 1
            break
        } else if offset == line.count-startIndex {
            endIndex = line.count - 1
        }
    }
    if endIndex == 0 { endIndex = line.count - 1}
    //print("\(startIndex) \(endIndex) \(line) \(currentIndex)")
    let relevant = line[startIndex...endIndex]
    let strValue = relevant.joined()
    guard let value = Int(strValue) else {
        print("\(strValue) not conversible to Int")
        return nil
    }
    return (value, IndexPair(x: startIndex, y: endIndex))
}
