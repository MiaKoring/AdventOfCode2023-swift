...
//  DayTwo
//
//  Created by Mia Koring on 23.10.24.
//
...
struct Game {
    ...
    private func isPossible () -> Bool {
        var isPossible: [Bool] = []
        for value in values {
            isPossible.append(value.red <= 12 && value.green <= 13 && value.blue <= 14)
        }
        return !isPossible.contains(false)
    }
    func possibleID () -> Int {
    ...
    func power() -> Int {
        ...
        return (red.last ?? 1) * (green.last ?? 1) * (blue.last ?? 1)
    }
}
...
func parseInput(_ input: String) -> [Game] {
    ...
    for line in lines {
        ...
        let gameStringID = parts[0].drop(while: {!$0.isWholeNumber})
        let rounds = parts[1].split(separator: ";")
        var game = Game(id: convertToInt(String(gameStringID)), values: [])
        for round in rounds {
            let values = round.split(separator: ",").map({String($0).replacingOccurrences(of: " ", with: "")})
            var rgbValues = RGB()
            for value in values {
                if value.contains("red") {
                    let number = String(value).replacingOccurrences(of: "red", with: "")
                    rgbValues.red = convertToInt(number)
                } else if value.contains("green") {
                    let number = String(value).replacingOccurrences(of: "green", with: "")
                    rgbValues.green = convertToInt(number)
                } else if value.contains("blue") {
                    let number = String(value).replacingOccurrences(of: "blue", with: "")
                    rgbValues.blue = convertToInt(number)
                }
            }
            game.values.append(rgbValues)
        }
        games.append(game)
    }
    return games
...
func convertToInt(_ input: String) -> Int {
    guard let int = Int(input) else {
        print("input not conversible to int: \(input)")
        return 0
    }
    return int
}
...
func main(first: Bool) {
    let games = parseInput(input)
    var count = 0
    for game in games {
        if first {
            count += game.possibleID()
            continue
        }
        count += game.power()
    }
