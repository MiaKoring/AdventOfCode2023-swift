...
//  DayOne
//
//  Created by Mia Koring on 22.10.24.
//
...
func calibrate(_ input: String) {
    let splitted: [String] = input.split(separator: "\n").map { String($0) }
    var res = 0
    for line in splitted {
        let first = line.first(where: {numberList.contains(String($0))})
        let last = line.last(where: {numberList.contains(String($0))})
